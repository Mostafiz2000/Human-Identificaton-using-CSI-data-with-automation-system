...
#define WAIT_TIMER_DURATION_MS 10000
/* The event group allows multiple bits for each event, but we only care about one event
 * - are we connected to the AP with an IP? */
const int WIFI_CONNECTED_BIT = BIT0;

static const char *TAG = "Active CSI collection (Station)";
void waitTimerCallback(void* arg) {
    // Perform actions when the wait timer event occurs
    // For example, disconnect from Wi-Fi and enter low-power mode
    esp_wifi_disconnect();
    // Enter low-power mode or perform other actions here
}
...
extern "C" void app_main() {
     esp_timer_handle_t waitTimerHandle;
    esp_timer_create_args_t waitTimerArgs = {
        .callback = waitTimerCallback,
        .arg = NULL,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "wait_timer"
    };
     esp_timer_create(&waitTimerArgs, &waitTimerHandle);
    esp_timer_start_once(waitTimerHandle, WAIT_TIMER_DURATION_MS * 1000);
