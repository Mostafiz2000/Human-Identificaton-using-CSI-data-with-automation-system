...
def setdata(a,b):
    ...
    # print([a])
    # data = [a]
    with open("experiment2.csv", "a", newline="") as f:
        writer = csv.writer(f)
...
def carrier_plot(amp,ti,phas,cnt):
    ...
    result=""
    if (Option=="3" and int(float(ti[0]))%3==0):
...
def process(res):
    # 
    global prev_opn,Option,inputstr,chek,prev_sum,all_data,temp_datak,temp_data
    ...
    all_data = res.split(',')
    if(Option=="4"):
        with open("experiment1.csv", "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(all_data)
    
    rssi_data=all_data[3]
    ...
    if(Option=="4" or Option=="3"):
        temp_data.clear()
        temp_data.append(rssi_data)
        temp_data.extend(csi_data)
        temp_datak=temp_data
        print(len(temp_datak))
        if(Option=="4"):
            temp_data.append(inputstr)
            with open("experiment2.csv", "a", newline="") as f:
                writer = csv.writer(f)
